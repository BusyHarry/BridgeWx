...
// ================================================================
// Scoring
// level        : 0..6 ( 1 bid to 7 bid)
// suit         : 0..4 (clubs, diamonds, hearts, spades, NoTrump)
// double       : 0..2 (no double, double, re-double)
// declarer     : 0..3 (Nord, East, South, West)
// vulnerability: 0..1 (no, yes)
// result       : -13..+6 (13 down-tricks to 6 up-tricks)
//
function CalcScore(level, suit, double, declarer, vulnerability, result) {
  level        = Number(level);	// force all parameters to a real number (if its a string)
  suit         = Number(suit);
  double       = Number(double);
  declarer     = Number(declarer);
  vulerability = Number(vulnerability);
  ...
  if ( declarer == -1 ) return 0; // a bye: no bid at all
  if ( 7+level+result < 0 )
  {
    result = -(7+level);  // can only loose as many tricks as the contract is!
  }
  if ( 7+level+result > 13 )
  {
    result = 13-(7+level);  // can only have 13 tricks!
  }
  ...
  return (declarer & 1)  ? -points : points;	// return score for NS
}
