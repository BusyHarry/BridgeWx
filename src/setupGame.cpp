...
SetupGame::SetupGame(wxWindow* a_pParent, UINT a_pageId) :Baseframe(a_pParent, a_pageId)
{
    ...
    // match description
    wxStaticText* txtDescription = new wxStaticText(this, wxID_ANY, _("Beschrijving:"));
    txtDescription->SetToolTip(_("Korte beschrijving van deze wedstrijd"));
...
// max mean percentage for session you have not played (in result over all sessions)
    ...
    wxStaticText* txtMaxMean = new wxStaticText(this, wxID_ANY, _("Maximum % bij afwezigheid:"));
    //xgettext:no-c-format
    txtMaxMean->SetToolTip(_("Maximum % dat je krijgt in de totaal uitslag voor een afwezige zitting"));
...
// minmax count for club result
    wxStaticText* txtMinMaxClub = new wxStaticText(this, wxID_ANY, _("Min/Max clubparen:"));
    ...
    txtMinMaxClub->SetToolTip(_("Minimum en maximum aantal paren om mee te tellen voor de clubuitslag"));
...
//    m_pChkBoxNeuberg = new wxCheckBox(this, wxID_ANY, "Neuberg");
    m_pChkBoxNeuberg->SetToolTip(_("Neuberg berekening bij arbitrale scores"));

// use 'weighted mean' of the result of each match when calculating result of N sessions
    m_pChkBoxWeightedMean = new wxCheckBox(this, wxID_ANY, _("Gewogen gemiddelde"));
    m_pChkBoxWeightedMean->SetToolTip(_("Gewogen gemiddelde berekening in uitslag over meerdere zittingen"));

    m_pChkBoxGroupResult = new wxCheckBox(this, wxID_ANY, _("Groepuitslag"));
    m_pChkBoxGroupResult->SetToolTip(_("Uitslag voor de groep op paarnummer en uitslagen per spel"));

    m_pChkBoxGlobalNames = new wxCheckBox(this, wxID_ANY, _("Centraal namen bestand"));
    m_pChkBoxGlobalNames->SetToolTip(_("Gebruik van een globaal namenbestand ipv namen per wedstrijd"));

    m_pChkBoxClock = new wxCheckBox(this, wxID_ANY, _("Klok"));
    m_pChkBoxClock->SetToolTip(_("Display van een klok in de statusbalk"));
    ...
    wxStaticBoxSizer* vBox = new wxStaticBoxSizer(wxVERTICAL, this, _("Algemene instellingen voor deze wedstrijd "));
...
void SetupGame::BackupData()
{
    ...
    if (cfg::GetGlobalNameUse() != bNewGlobalNameUse)
    {
        ...
        {
            MyMessageBox(_("Wisselen van/naar centrale namen, terwijl er al paren zijn!"), _("Waarschuwing"));
        }
    }
}   // BackupData()
...
{
    bool bResult = prn::BeginPrint(_("Wedstrijdinstellingen pagina:\n")); MY_UNUSED(bResult);
    wxString info;
    info = FMT(_( "Beschrijving      : %s\n"
                  "Max %% afwezig     : %s\n"
