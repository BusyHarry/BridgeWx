...
private:
    ...
    void GetCurrentValueAndInsertionPoint(wxString& val, int& pos)const;    // retrieve current textvalue and insertion point from the txtCtrl
    bool IsCharOk           (const wxString& val, int pos, wxChar chr)const;// is 'chr' acceptable at this position
    bool IsExtraChar        (wxChar chr) const;                             // true, if 'chr' is in 'm_sStartChars'
...
public:
    explicit MyGridCellEditorWithValidator(double min, double max, int precision = -1);
             MyGridCellEditorWithValidator() = default;
            ~MyGridCellEditorWithValidator() override = default;
    void     BeginEdit                    (int row, int col, wxGrid* grid) override;  // needed to get attached wxTextCtrl
    bool     IsAcceptedKey                (wxKeyEvent& event) override;                        // check if key can start edit
private:
...
public:
    MyTextCtrlWithValidator(wxWindow *parent, wxWindowID id,
        ...
        long                  style = 0);
   ~MyTextCtrlWithValidator() override = default;
private:
