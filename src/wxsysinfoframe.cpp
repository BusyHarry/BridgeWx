...
#define COLUMN_WIDTH(col) (GetColumnWidth(col)/GetCharWidth()+3) /*estimated guess*/
...
{
    return wxString::Format("%d x %d", s.x, s.y);
}
...
wxArrayString SysInfoListView::GetNameAndValueValues(int nameColumnIndex, int valueColumnIndex, const wxString& separator) const
{
    ...
    values.reserve(itemCount + (size_t)1);
...
{
public:
    explicit SystemSettingView(wxWindow* parent);
...
{
public:
    explicit SystemColourView(wxWindow* parent);
    ~SystemColourView();
...
{
public:
    explicit SystemFontView(wxWindow* parent);
...
{
public:
    explicit SystemMetricView(wxWindow* parent);
protected:
    void DoUpdateValues() override;
};
...
{
public:
    explicit DisplaysView(wxWindow* parent);
...
wxArrayString DisplaysView::GetValues(const wxString& separator) const
{
    ...
    values.reserve(itemCount + (size_t)1);
    // column headings
    s.Printf("%-*s", COLUMN_WIDTH(0),"Parameter");
    for ( int columnIndex = 1; columnIndex < columnCount; ++columnIndex )
    {
        ...
        s += wxString::Format("%*s%-*s", (int)separator.Len(), "", COLUMN_WIDTH(columnIndex), listItem.GetText());
    }
    values.push_back(s.Trim());

    // dump values
    for ( int itemIndex = 0; itemIndex < itemCount; ++itemIndex )
    {
        s = wxString::Format("%-*s", COLUMN_WIDTH(0), GetItemText(itemIndex, 0));
        for ( int columnIndex = 1; columnIndex < columnCount; ++columnIndex )
        {
            s += separator + wxString::Format("%-*s",COLUMN_WIDTH(columnIndex), GetItemText(itemIndex, columnIndex));
        }
        values.push_back(s);
    }
...
{
public:
    explicit SystemOptionsView(wxWindow* parent);
...
{
public:
    explicit StandardPathsView(wxWindow* parent);
...
{
public:
    explicit EnvironmentVariablesView(wxWindow* parent);
...
{
public:
    explicit MiscellaneousView(wxWindow* parent);
    ~MiscellaneousView();
...
{
public:
    explicit ObtainFullHostNameThread(wxEvtHandler* sink)
...
void MiscellaneousView::DoUpdateValues()
{
    ...
    for ( int i = 0; i < itemCount; ++i )
    {
        ...
        switch ( param )
        {
            ...
            case Param_WindowDPI:                 value.Printf("%u", MSWDPIAwarenessHelper::GetDpiForWindow(this)); break;
#endif // #ifdef __WXMSW__
...
{
public:
    explicit PreprocessorDefinesView(wxWindow* parent);
...
void PreprocessorDefinesView::DoUpdateValues()
{
    ...
    bool hasDefine = false; // just init var...
                                      ...
                                      long frameStyle, long createFlags)
{
    ...
    LogInformation("wxSystemInformationFrame, (c) PB: https://github.com/PBfordev");
...
WXLRESULT wxSystemInformationFrame::MSWWindowProc(WXUINT nMsg, WXWPARAM wParam, WXLPARAM lParam)
{
    if ( nMsg == WM_SETTINGCHANGE )
    {
        LogInformation(wxString::Format("WM_SETTINGCHANGE received: wParam = %u, lParam =\"%s\"",
            static_cast<unsigned>(wParam), lParam ?  reinterpret_cast<LPCTSTR>(lParam) : wxS("")));
        TriggerValuesUpdate();
    }
